...
void controlServo(float distance)
{
  ...
  {
    int angle = map(distance, 5, 50, servoMin, servoMax);
...
void loop()
{
  ...
  // ----- CONTROL OUTPUTS -----
  //add functions that you write here and comment the once you dont want to use

  controlBuzzer(distance); //buzzer sounds when within 10cm of distence sensor
  controlServo(potValue); // potentiometer controls servo movment
  controlRGBfromAccel(ax, ay, az); // lights up RGB led baced on accelerometer orientation
