...
#Unpack Mocap Data Functions
    ...
    def __command_thread_function( self, in_socket, stop, gprint_level, command_q):
        message_id_dict={}
                ...
                if message_id == self.NAT_FRAMEOFDATA:
                    if print_level > 0:
                        if (message_id_dict[tmp_str] % print_level) == 0:
                            ...
                            print_level = 0
                message_id = self.__process_message( data , command_q, print_level)
    ...
    def __data_thread_function( self, in_socket, stop, gprint_level, data_q):
        message_id_dict={}
        data=bytearray(0)
        # 64k buffer size
        recv_buffer_size=64*1024
                ...
                if message_id == self.NAT_FRAMEOFDATA:
                    if print_level > 0:
                        if (message_id_dict[tmp_str] % print_level) == 0:
                            ...
                            print_level = 0
                message_id = self.__process_message( data ,data_q, print_level)

                data=bytearray(0)
        return 0

    def __process_message( self, data : bytes, q, print_level=0):
        #return message ID
    ...
    def run( self , data_q, command_q):
        ...
        self.data_thread = Thread( target = self.__data_thread_function, args = (self.data_socket, lambda : self.stop_threads, lambda : self.print_level, data_q))
        self.data_thread.start()

        # Create a separate thread for receiving command packets
        self.command_thread = Thread( target = self.__command_thread_function, args = (self.command_socket, lambda : self.stop_threads, lambda : self.print_level, command_q))
