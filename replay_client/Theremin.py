...
Usage:
    python -m replay_client.Theremin --config theremin.json \\
        --isadora-ip 127.0.0.1 \\
        --isadora-port 1234 \\
        --speed 1.0

Example config (theremin.json):
    {
        "scene_1": {
            "start_time": 0.0,
            "csvs": [
...
log = logging.getLogger("theremin")
...
class CSVConfig:
    ...
    skeleton_bones: Optional[List[str]] = field(default_factory=lambda: ["RThumb", "LThumb", "LShoulder"])
        ...
        def _parse_csv_list(csv_data: List[dict]) -> List[CSVConfig]:
            return [
                CSVConfig(
                    ...
                    skeleton_bones=item.get('skeleton_bones', ["RThumb", "LThumb", "LShoulder"]),
...
def main():
    """Main entry point for theremin driver."""
    args = parse_args()
    ...
    log.info("=" * 70)
    log.info("THEREMIN DRIVER - Multi-CSV Synchronization")
    log.info("=" * 70)
